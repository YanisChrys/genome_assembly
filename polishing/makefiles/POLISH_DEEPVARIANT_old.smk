...
rule all: 
    input:
    # polishing
        expand("RESULTS/ASM_POLISHING_DEEPVARIANT/FINAL/{sample}_" + run + ".fasta",sample=samples),
    # stats
...
rule align_reads:
    ...
    output:
        bam=temp("RESULTS/ASM_POLISHING_DEEPVARIANT/alignments/{sample}_" + run + ".bam")
    threads:
        workflow.cores
    conda:
        "../envs/pbmm2.yaml"
...
rule index_chromosomes:
    input:
        "RESULTS/ASM_POLISHING_DEEPVARIANT/genome_split/chromosomes/{sample}_" + run + "/{chrom}.fasta"
    output:
        "RESULTS/ASM_POLISHING_DEEPVARIANT/genome_split/chromosomes/{sample}_" + run + "/{chrom}.fasta.fai"
    conda:
        "../envs/samtools.yaml"
    threads: 
...
rule create_bed:
    input:
        "RESULTS/ASM_POLISHING_DEEPVARIANT/genome_split/chromosomes/{sample}_" + run + "/{chrom}.fasta.fai"
    output:
        temp("RESULTS/ASM_POLISHING_DEEPVARIANT/genome_split/chromosomes/{sample}_" + run + "/{chrom}.bed")
    threads: 
        min(workflow.cores,5)
    shell: """
        awk '{{printf("%s\\t0\\t%s\\n",$1,$2)}}' {input}  > {output}
...
rule split_aligned_reads:
    input:
        bam="RESULTS/ASM_POLISHING_DEEPVARIANT/alignments/{sample}_" + run + ".bam",
        bed="RESULTS/ASM_POLISHING_DEEPVARIANT/genome_split/chromosomes/{sample}_" + run + "/{chrom}.bed"
    output:
        temp("RESULTS/ASM_POLISHING_DEEPVARIANT/alignments_split/{sample}_" + run + "_{chrom}_filt.bam")
    threads: 
        min(workflow.cores,5)
    conda:
        "../envs/samtools.yaml"
...
rule run_deepvariant:
    input:
        ref = "RESULTS/ASM_POLISHING_DEEPVARIANT/genome_split/chromosomes/{sample}_" + run + "/{chrom}.fasta",
        bam = "RESULTS/ASM_POLISHING_DEEPVARIANT/alignments_split/{sample}_" + run + "_{chrom}_filt.bam"
    output:
        vcf = "RESULTS/ASM_POLISHING_DEEPVARIANT/deepvariant/{sample}_" + run + "/{chrom}_all.filt.dv.vcf"
    threads: 
        min(workflow.cores,10)
    container:
        "/share/scientific_bin/singularity/containers/deepvariant_1.2.0.sif"
    params:
        "RESULTS/ASM_POLISHING_DEEPVARIANT/deepvariant/{sample}_" + run + "/output/intermediate_files/{chrom}/"
    shell: """ 
        run_deepvariant \
        --model_type=PACBIO \
        --ref={input.ref} \
...
rule filter_deepvariant:
    input:
        chrom="RESULTS/ASM_POLISHING_DEEPVARIANT/genome_split/chromosomes/{sample}_" + run + "/{chrom}.fasta",
        vcf="RESULTS/ASM_POLISHING_DEEPVARIANT/deepvariant/{sample}_" + run + "/{chrom}_all.filt.dv.vcf"
    output:
        vcf="RESULTS/ASM_POLISHING_DEEPVARIANT/deepvariant_filtered/{sample}_" + run + "/{chrom}_all.filt.dv.filt.vcf.gz"
    conda:
        "../envs/bcftools.yaml"
    threads: 
        min(workflow.cores,5)
    shell: """
        bcftools view -f 'PASS' -i 'GT="1/1"' {input.vcf} -Oz -o {output.vcf} --threads {threads}
        tabix -p vcf {output.vcf}
    """

rule create_consensus_fasta:
    input:
        fasta="RESULTS/ASM_POLISHING_DEEPVARIANT/genome_split/chromosomes/{sample}_" + run + "/{chrom}.fasta",
        vcf="RESULTS/ASM_POLISHING_DEEPVARIANT/deepvariant_filtered/{sample}_" + run + "/{chrom}_all.filt.dv.filt.vcf.gz"
    output:
        "RESULTS/ASM_POLISHING_DEEPVARIANT/chrom_consensus/{sample}_" + run + "/{chrom}.consensus.fasta"
    conda:
        "../envs/bcftools.yaml"
    threads: 
        min(workflow.cores,5)
...
# def get_chromosomes_for_sample(wildcards):
# interval_list_path = f"RESULTS/ASM_POLISHING_DEEPVARIANT/StrUra_202401_m64078e_220520_154130_64078e_220605_062234_hifiasm_incpert_D10N150_l2_alternate_yahs_scaffolds_final_deepvariant/interval.list"
#     try:
# with open(interval_list_path, 'r') as file:
#     chroms = file.read().splitlines()
#         return chroms
...
rule merge_chroms:
    input: 
        expand("RESULTS/ASM_POLISHING_DEEPVARIANT/chrom_consensus/{{sample}}_" + run + "/{chrom}.consensus.fasta",
        chrom=get_chromosomes_for_sample)
    output: 
